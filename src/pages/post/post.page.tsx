...
const Post: FC<PostProps> = () => {
   ...
   const submitComment = async () => {
      ...
      setTimeout(async () => {
         // react is doing some insance optimization thats not letting my notification components refresh. added artificial delay
         if (user === null) {
         ...
         if (user === 'loading') {
            setNotifs([
               { msg: `can't validate user. Try again in a few secods`, status: 'error' },
            ]);
            setComment('');
            setPublishingComment(false);
            return;
         ...
         if (errors.length) {
            setNotifs(
               errors.map<NotificationConfig>(m => {
                  return { msg: m, status: 'error' };
               })
            );
         } else {
            try {
               ...
               .then(pData => {
                  ...
                  fetchDoc<myUser>('users', pData.author)
                     .then(aData => {
                        // author found!
                        setAuthor(aData);
                        fetchImgUrl(`pic_${aData.pic}.png`).then(iPath =>
                           setAuthorPic(iPath)
                        );
                     })
                     .catch(() => {
                        // author not found :(
                        setAuthor('deleted');
                        fetchImgUrl(`pic_${0}.png`).then(iPath => setAuthorPic(iPath));
                     });
               })
               .catch(e => {
                  // post not found
   ...
   const handleVote = async (rating: Rating) => {
      ...
      try {
         if (uid) {
            ...
            setNotifs([]);
            setTimeout(
               () => setNotifs([{ msg: 'signin to vote', status: 'error' }]),
               100
            );
         }
      } catch (e) {
   ...
   if (post === 'not found') {
      // TODO: Add styles
      return (
         <div className='flex flex-col gap-4 justify-center items-center place-content-center select-none'>
            <div className='text-4xl font-bold p-7 text-regular text-center'>
               Post is either deleted or never existed :(
            </div>
            <div
               className='text-xl text-center bg-primary text-normal rounded-sm w-auto font-medium leading-none p-2'
               onClick={() => nav('/feed')}>
               take me back
            </div>
         </div>
      ...
      // Loading data
      return (
         <CgSpinner className='animate-spin text-normal w-16 h-16 place-self-center' />
      );
   } else {
      // Loaded
      return (
         <div className='post-cnt'>
            ...
            <div className='post-sub-heading'>
               <div
                  ...
                  onClick={() => {
                     if (author && author !== 'deleted') nav(`/user/${author.uid}`);
                  }}>
                  {
                     author === 'deleted' ? (
                        author
            ...
            <div className='flex justify-center'>
               <div className='post-votes'>
                  <ImArrowUp
                     ...
                     className={`post-vote-icon ${
                        vote === 'liked' ? 'text-primary' : ''
                     }`}
                  />
                  ...
                  <ImArrowUp
                     ...
                     className={`post-vote-icon transform rotate-180 ${
                        vote === 'disliked' ? 'text-primary' : ''
                     }`}
                  />
               </div>
            </div>
            ...
            <SubmitButton
               ...
               load={publishingComment}
               Icon={MdPostAdd}
               extra={'post-submit-comment-btn'}
            />
            {notifs.length ? <Notification msgs={notifs} delay={3000} /> : <></>}
            <div className='post-comments'>
               {comments.map((c, i) => (
                  <CommentCard data={c} key={i} />
               ))}
            </div>
            {!comments.length ? (
               <div
            ...
            '>
               ...
               <div
                  ...
                  flex justify-center items-center rounded-md self-center place-self-center mt-1 select-none
               '
