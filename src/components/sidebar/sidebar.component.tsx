...
interface SidebarProps { state?: boolean }
...
const Sidebar: FC<SidebarProps> = ({ state }) => {
   ...
   let [ gracePeriod, setGracePeriod ] = useState<boolean>(false);
   ...
   return <>
      <div 
         ...
         <div className="sb-acct-area">
            <div className={`sb-acct-top ${user && user !== 'loading' ? 'cursor-pointer' : ''}`} onClick={goToProfile}>
               {
                  picURL ? <img src={picURL} className="sb-user-pic" alt="idk"/> : <div className="sb-user-pic-loading"/>
               }
               {
                  user === 'loading' ? <div className="sb-user-name-loading"/> :
                  !user ? <div className="sb-user-name-out"> { notLoggedInName } </div> :
                  <div className="sb-user-name"> { user.username } </div>
               }
            </div>
            <div className="sb-acct-bot">
               {
                  user === 'loading' ? <div className="sb-user-link-loading"/> :
                  !user ? 
                     <div className="sb-user-link-loggedout" onClick={signIn}>
                        <div className="flex flex-row items-center group select-none gap-1 group cursor-pointer">
                           ...
                           <div className="sb-acct-bot-text">
                              signin / signup
                           </div>
                        </div>
                     </div> :
                     <div className="sb-user-link-loggedin">
                        <div className="flex flex-row items-center group select-none gap-1 group cursor-pointer" onClick={signOut}>
                           <IoLogOut className="sb-user-icon"/>
                           <div className="sb-acct-bot-text">
                              signout
                           </div>
         ...
         <SidebarItem text='POST' icon={ MdOutlinePostAdd } path='/new-post' callback={toggle}/>
         <SidebarItem text='PROFILE' icon={ CgProfile } path='/me' callback={toggle}/>
         <SidebarItem text='ABOUT' icon={ AiOutlineQuestionCircle  } path='/about' callback={toggle}/>
